...
const clientSignupSchema = refinedBaseSchema
  .extend({
    ...
    activityType: z.enum(
      ...
      {
        message:
          "activity type must be one of the following: Wedding hall, Restaurant, Cafe, Club, Other",
      }
    ),
    activityName: z
      .string()
    ...
    customBusinessType: z
      .string()
      .max(255, "Custom business type too long")
      .optional(),
  })
  .refine(
    (data) => {
  ...
  .refine(
    ...
    {
      message:
        "Custom business type should only be provided when 'Other' is selected",
      path: ["customBusinessType"],
    }
  );
